...
from results import pull_results, get_best_algorithm_hyperparameter_onestep
...
def perform_model_selection(evaluate_best_models=True):
    ...
    results_df = pull_results(ignition_ids=['1','2','4','5','6','10','15','16','17','18','19','20','21'])
    ...
    best_models = choose_models_with_recall(models_df=best_df,
    ...
    train_best_models_mp(X_train, y_train, best_models=best_models, prod_config=prod_config, local_paths=local_paths, cores=3)

    if evaluate_best_models:

        # Test best models for each review group
        scored_papers_test = score_papers(X_test, prod_config, models_path=local_paths['store_production_models'])
        save(object=scored_papers_test, location=local_paths['store_scored_papers'], filename='scored_papers_citations')
        ...
        upper_thresholds, lower_thresholds, recall_at_thresholds = get_thresholds(y_test, y_pred_test, minimum_precision=0.95, minimum_recall=0.99)
        ...
        for review_group in tqdm(prod_config['review_groups_recall'].keys(), desc='Review Group'):
            ...
            for recall in tqdm(prod_config['recalls'], desc='Evaluations'):
                ...
                precisions = evaluate_precision_at_k_recall(class_true=y_test, class_prob=y_pred_test, k=recall)

                # store results in database
                production_results_to_db(table_name=prod_config['results_table_name'],
                                            unique_id=f"{rg}_{recall}",
                                            review_group=rg,
                                            algorithm=best_models[review_group]['algorithm'],
                                            hyperparameters=best_models[review_group]['hyperparameters'],
                                            recall=recall,
