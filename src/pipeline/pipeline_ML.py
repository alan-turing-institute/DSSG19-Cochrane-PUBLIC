...
def run_pipeline(ignition_file, persist_all, load_all_fresh):
    ...
    if check_persisted(local_paths_env['store_train_data'], f'{hash_id}_x', load_all_fresh):
        ...
        X_train, X_test, y_train, y_test = sample(ignition, connection, local_paths_env['store_features'])
    ...
    for fold in tqdm(range(ignition['k_folds']), desc='Folds'):
        ...
        for hyperparam in tqdm(hyperparameters, desc='Hyperparameters'):
            ...
            classifier = select_classifier(ignition["model_type"], fold_id, ignition["target"],
                        ignition["classes"], fold_features, hyperparameters=hyperparam,
                        seed=ignition['seed'], env=local_paths_env, load_fresh=load_all_fresh)
